...
## DONE - add ward capacity
## DONE - add fallback to less-preferred wards
## TODO - allow patient to have a set "current" ward rather than dropping and immediately seizing again (nb prio shows how good we think the present ward is)
## TODO - move initial ward handling into emergency and elective start trajectories, so common only handles things thereafter
## TODO - handle elective patients properly (ie don't let them in when no ward is available)
...
simmer_wrapper <- function(i) {
  ...
  delay_and_reset<-trajectory() %>% 
    log_("no beds at all!") %>% 
    timeout(3600) %>% 
    set_attribute("pr",1) %>% 
    rollback(6) #5 if both log statements removed (ie one from patient_rejected also counts), 7 with both present, 6 with one
  
   
  patient_rejected<-trajectory() %>% 
    #log_("rejected") %>%
    branch(function() {
      if (get_attribute(env,"pr")<4) 0 else 1
    },continue=FALSE,delay_and_reset) %>% 
    set_attribute("pr",1,mod="+") %>% 
    rollback(4) #5 with, 4 without log statement
  ...
  common_patient<-trajectory() %>% 
    ...
    rollback(6,Inf,function() {now(env)<get_attribute(env,"end_time_spell")}) %>% 
