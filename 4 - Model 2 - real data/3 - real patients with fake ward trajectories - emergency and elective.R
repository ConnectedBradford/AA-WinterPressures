...
simmer_wrapper <- function(i) {
  ...
  emergency_patient<- trajectory() %>%
    ...
    release_selected() %>% 
    rollback(5,Inf,function() {now(env)<get_attribute(env,"end_time")}) %>% 
  ...
  elective_patient<- trajectory() %>%
    ...
    set_attribute("cur_traj",1) %>% 
    set_attribute("start_time", function() {now(env)}) %>% 
    set_attribute("end_time", function() { as.numeric(elective_spells[get_attribute(env,"sp_row_id"),"duration"])+now(env) }) %>% 
    seize("bed") %>% 
    #log_("In bed") %>% 
    # timeout(function() { as.numeric(elective_spells[get_attribute(env,"sp_row_id"),"duration"]) }) %>% 
    select(function() {traj_pr1[[get_attribute(env,"cur_traj")]]},"first-available") %>% 
    ## above - could concatenate pr1,pr2,pr3? Then afterwards raise flags depending on which we got?
    ## or add a "FAIL" resource to the end of each list with infinite capacity and check if we've selected that before adding more?
    seize_selected() %>% 
  ...
  env %>% 
    ...
    add_dataframe("Emergency Patient",emergency_patient,emergency_table,col_time="gaps",time="interarrival",col_attributes="sp_row_id") %>% 
    add_dataframe("Elective Patient",elective_patient,elective_table,col_time="gaps",time="interarrival",col_attributes="sp_row_id") %>% 
