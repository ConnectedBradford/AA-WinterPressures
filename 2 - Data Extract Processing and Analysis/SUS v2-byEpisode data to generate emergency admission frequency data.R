...
spells<-filter(data,`_Elective`==FALSE)
...
mainout<-spells[output,on=.(`_bizday`=`bizDay`,`_Start_Time`>=`startTSearch`,`_Start_Time`<=`endTSearch`,`_2KMD_Date`>=`startDTSearch`,`_2KMD_Date`<=`endDTSearch`),j=.(.N,dateTime),by=.EACHI,allow.cartesian=TRUE]
...
for (toffset in c(-86400,0,86400)) {
  for (doffset in c(as.difftime(-365,units="days"),0,as.difftime(365,units="days"))) {
    ...
    tmpcount<-spells[tempout,on=.(`_bizday`=`bizDay`,`_Start_Time`>=`startTSearch`,`_Start_Time`<=`endTSearch`,`_2KMD_Date`>=`startDTSearch`,`_2KMD_Date`<=`endDTSearch`),j=.(.N,dateTime),by=.EACHI,allow.cartesian=TRUE]
...
## now correct to a daily rate (presently depends on length of search time window)
mainout<-mutate(mainout,`_correctedN`=`_correctedDaysN`*(12/as.numeric(searchTimeWindow)))



print("* Corrected for data availability *")

saveRDS(mainout,"../Data - For Modelling/Emergency-Frequency.rds")
saveRDS(spells,"../Data - For Modelling/Emergency-Spells.rds")
