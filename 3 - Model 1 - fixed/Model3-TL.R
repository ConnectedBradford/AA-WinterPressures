...
wards <- readRDS("../Data - Generated/wardstays_wards.rds")
patients <- readRDS("../Data - Generated/wardstays_nested.rds")
...
patient<- trajectory() %>%
  ...
  set_attribute("sex", function() {sample(c(1,2),1)}) %>% 
  set_attribute("specialtycode", function() {sample(c(100,101,120,300,301,320),1)}) %>% 
  seize("bed") %>% 
  ...
  release_selected() %>% 
  set_attribute("segment",1,"+") %>% 
  rollback(5,check = function() {!is.na(patients[["data"]][[get_attribute(env,"patient_id")]][["req"]][get_attribute(env,"segment")])}) %>% 
  release("bed")
