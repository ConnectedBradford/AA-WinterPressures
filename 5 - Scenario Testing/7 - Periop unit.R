...
wards<-add_row(wards,Ward="Periop",Beds=Inf)
...
simmer_wrapper <- function(i) {
  ...
  CC_admission<-trajectory() %>% 
    ...
    select(function() {
      if (critcare_segments[get_attribute(env,"nxt_cc_row_id"),"periop"]) return("Periop")
  ...
  elective_CC_patient<- trajectory() %>%
    ...
    select(function() { 
      if (critcare_segments[get_attribute(env,"cc1_row_id"),"periop"]) return("Periop") 
...
saveRDS(resources,"../Model Outputs/7-resources.rds")
saveRDS(attribs,"../Model Outputs/7-attribs.rds")
saveRDS(arrivals,"../Model Outputs/7-arrivals.rds")
