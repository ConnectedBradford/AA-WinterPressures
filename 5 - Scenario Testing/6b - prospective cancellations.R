## Scenario 6 - prospective cancellations/rescheduling

## Move elective patients from a predictably busy time of year to a less busy one.
## From baseline - Start June to end september appears quieter (June July August September)
## April/May and October/November probably worst
## Try moving April/May surgery to June/July
## And October/November surgery to August/September
## Plan b - spread that surgery over the period instead ie April/May spread over April-July, and October/November spread over August-November (by randomly choosing 50% of patients to move)
...
simmer_wrapper <- function(i) {
  ...
  elective_table<-
    elective_table %>%
    mutate(time_at=if_else((month(idx_datetime)==4) & (runif(n())>0.5),time_at+2*28*24*3600,time_at)) %>%
    mutate(time_at=if_else((month(idx_datetime)==5) & (runif(n())>0.5),time_at+2*28*24*3600,time_at)) %>%
    mutate(time_at=if_else((month(idx_datetime)==10) & (runif(n())>0.5),time_at-2*28*24*3600,time_at)) %>%
    mutate(time_at=if_else((month(idx_datetime)==11) & (runif(n())>0.5),time_at-2*28*24*3600,time_at))
  ...
  ##recalculate gaps (as that's what drives simmer)
   ...
   elective_table<-
     elective_table %>% 
     arrange(time_at) %>% 
     mutate(gaps=time_at-lag(time_at,default=0))
   
   
   print ("* modifications for prospective rescheduling done *")
...
saveRDS(resources,"../Model Outputs/6b-resources.rds")
saveRDS(attribs,"../Model Outputs/6b-attribs.rds")
saveRDS(arrivals,"../Model Outputs/6b-arrivals.rds")
