## Scenario 3 - Infinite Wards

## Wards have infinite capacity, also track trajectory type through a run (as an extra resource)
...
simmer_wrapper <- function(i) {
  ...
  ##rollback to timeout in common_patient (added3)
  ...
  CC_discharge<-trajectory() %>% 
    ...
    #log_("cc discharge") %>% 
    rollback(7) ## to timeout in common_patient (added3)
  ...
  common_patient<-trajectory() %>% 
    ...
    seize("bed") %>% 
    select("Traj-Init",id=5) %>% 
    seize_selected(id=5) %>% 
    release_selected(id=5) %>% ##rollbacks should go to here rather than timeout - ie add 3 to anything that was rolling back to timeout
    select(function(){
      paste0("Traj-",toString(get_attribute(env,"cur_traj")))
    },id=5) %>% 
    seize_selected(id=5) %>% 
    ...
    rollback(8,Inf,function() {now(env)<get_attribute(env,"end_time_spell")}) %>% ## to timeout (added3)
    ...
    ##log_("leaving") %>% 
    release_selected() %>% 
    release_selected(id=5) %>% #give up trajectory resource
    release("bed")
  #log_("Out of bed")
  ...
  env %>% 
   ...
   # add_generator("Ward Controller",ward_control,at(as.numeric(elective_freq[1,]$dateTime)+7*3600)) %>%  ## start 7am on first day
    ...
    for (i in 1:nrow(wards)) {
      add_resource(env,wards[i,"Ward"],capacity=Inf,queue_size=Inf,queue_priority=c(i*10,i*10+9))
  ...
  for (i in 1:length(traj_pr1)) {
    if (!is.null(traj_pr1[[i]])) {
      add_resource(env,paste0("Traj-",toString(i)),capacity=Inf)
    }
  }
  
  add_resource(env,"Traj-Init",capacity=Inf)
...
envs<-pbmclapply(1:24,simmer_wrapper,mc.cores=4)
...
for (i in 1:length(traj_pr1)) {
  if (!is.null(traj_pr1[[i]])) {
    trajectories<-c(trajectories,paste0("Traj-",toString(i)))
  }
}
...
#print(plot(resources,metric="usage",trajectories,steps=TRUE,items=c("server","queue")))
