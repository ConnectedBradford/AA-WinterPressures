## Scenario 5 - hotel for delayed discharges
...
emergency_spells$`_DischargeReady_DateTime`<-as.POSIXct(strptime(paste(emergency_spells$`Hospital Provider Spell Discharge Ready Date`,"10:00:00",sep=" "),format="%Y%m%d %H:%M:%S"))
elective_spells$`_DischargeReady_DateTime`<-as.POSIXct(strptime(paste(elective_spells$`Hospital Provider Spell Discharge Ready Date`,"10:00:00",sep=" "),format="%Y%m%d %H:%M:%S"))

emergency_spells$dr_duration<-pmax(as.numeric(emergency_spells$`_DischargeReady_DateTime`)-as.numeric(emergency_spells$`_SpellStart_DateTime`),0)
...
simmer_wrapper <- function(i) {
  ...
  discharge_to_hotel<-trajectory() %>% 
    seize("Hotel") %>% 
    release("bed") %>% 
    select(function() { wards$Ward[get_attribute(env,"cur_ward_idx")]  }) %>% 
    release_selected() %>% 
    timeout(function() {
      max(0,get_attribute(env,"end_time_spell")-now(env))
    }) %>% 
    release("Hotel")
  ...
  common_patient<-trajectory() %>% 
    ...
    timeout(function() { 
      max(0,
       min(traj_dur[[get_attribute(env,"cur_traj")]],
        ...
        get_attribute(env,"dr_time_spell")-now(env)
      )
      )
    ...
    branch(function() {
      if (now(env)>=get_attribute(env,"dr_time_spell") && now(env)<get_attribute(env,"end_time_spell")) 1 else 0
    },continue=FALSE,discharge_to_hotel) %>% 
    rollback(6,Inf,function() {now(env)<get_attribute(env,"end_time_spell")}) %>% ## to timeout
  ...
  emergency_CC_patient<- trajectory() %>%
    ...
    set_attribute("dr_time_spell", function() { as.numeric(emergency_spells[get_attribute(env,"sp_row_id"),"dr_duration"])+now(env) }) %>% 
  ...
  elective_CC_patient<- trajectory() %>%
    ...
    set_attribute("dr_time_spell", function() { as.numeric(elective_spells[get_attribute(env,"sp_row_id"),"dr_duration"])+now(env) }) %>% 
    join(set_patient_attributes_common_times) %>% 
  ...
  env %>% 
    ...
    add_resource("Hotel",capacity=Inf)
...
saveRDS(resources,"../Model Outputs/5-resources.rds")
saveRDS(attribs,"../Model Outputs/5-attribs.rds")
saveRDS(arrivals,"../Model Outputs/5-arrivals.rds")
...
resources2<-filter(resources,resource=="Hotel")
