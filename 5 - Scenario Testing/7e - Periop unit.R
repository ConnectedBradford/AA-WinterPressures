## Scenario 7e - periop unit

## 8 beds - 4 taken from 21 (non-monitored), 2 from ward 18 (ie net +2 beds)
...
##periop unit for elective patients
...
elective_spells$periop[oesoph_bl]<-2

## Periop codes
## 0 = not relevant
## 1 = urology/vascular/medium general - 24hr on periop unit then go to 26/14/HDU as normal
## 2 = oesophagectomy - go to HDU but step down to periop unit instead of 21


print("* Periop flag added to elective data *")


#critcare_segments_bak<-critcare_segments
critcare_segments$periop<-FALSE
...
for (i in 1:nrow(periop_spells)) {
  if (periop_spells[i,"cc1_row_id"]>0) {
    ...
    if (critcare_segments[cc1,"_SegmentStart_Offset"]<48*3600) {
      ...
      ## came, but later. Create a new periop stay at the start
      row_id<-nrow(critcare_segments)+1
      critcare_segments<-add_row(critcare_segments, `_TLSpellDigest`=periop_spells[i,"_TLSpellDigest"],
                                 `_SegmentStart_Offset`=0,
                                 `_SegmentEnd_Offset`=periopStay,
    ...
    ## they didn't come to CC at all - create them a stay
    row_id<-nrow(critcare_segments)+1
    critcare_segments<-add_row(critcare_segments, `_TLSpellDigest`=periop_spells[i,"_TLSpellDigest"],
                               `_SegmentStart_Offset`=0,
                               `_SegmentEnd_Offset`=periopStay,
...
## Oesophagectomies
## Find first "not real" CC stay and convert it to a periop stay

oesoph_spells<-filter(elective_spells,periop==2)
for (i in 1:nrow(oesoph_spells)) {
  cc<-oesoph_spells[i,"cc1_row_id"]
  while ((cc>0) && critcare_segments[cc,"_RealCritCare"]) {
    cc<-critcare_segments[cc,"segN_row_id"]
    if (is.na(cc)) cc<-0
  }
  while ((cc>0) && (!critcare_segments[cc,"_RealCritCare"])) {
...
print("* Periop loop completed *")


source("Trajectories.R")
source("Wards.R")

##reduce 21CC capacity and add periop unit
wards$Beds[wards$Ward=="21"]<-12 ## reduced by 4
wards$Beds[wards$Ward=="18"]<-22 ## reduced by 2

wards<-add_row(wards,Ward="Periop",Beds=8)
allwards<-wards$Ward[1:length(wards$Ward)-1]
...
simmer_wrapper <- function(i) {
  ...
  CC_admission<-trajectory() %>% 
    ...
    select(function() {
      if (critcare_segments[get_attribute(env,"nxt_cc_row_id"),"periop"]) return(c("Periop","ICU"))
      if (critcare_segments[get_attribute(env,"nxt_cc_row_id"),"_RealCritCare"]) "ICU" else c("21CC","ICU") },"first-available") %>% 
    branch(function() {
  ...
  CC_transfer<-trajectory() %>% 
    ...
    select(function() { 
      if (critcare_segments[get_attribute(env,"nxt_cc_row_id"),"periop"]) return(c("Periop","ICU"))
      if (critcare_segments[get_attribute(env,"nxt_cc_row_id"),"_RealCritCare"]) "ICU" else "21CC" },"first-available") %>% 
    branch(function() {
  ...
  emergency_patient<- trajectory() %>%
    ...
    select(function() {
      ...
      switch(pr,
             ...
             traj_pr2[[get_attribute(env,"cur_traj")]],
             traj_pr3[[get_attribute(env,"cur_traj")]],
             allwards) ##changed from baseline as we don't want to include periop unit
    },"shortest-queue-available") %>% 
  ...
  elective_CC_patient<- trajectory() %>%
    ...
    select(function() { 
      if (critcare_segments[get_attribute(env,"cc1_row_id"),"periop"]) return(c("Periop","ICU")) 
      if (critcare_segments[get_attribute(env,"cc1_row_id"),"_RealCritCare"]) "ICU" else c("21CC","ICU") },"first-available") %>% 
    set_prioritization(function() {  c(match(get_selected(env),wards$Ward)*10,-1,-1) }) %>% ## allow queueing
    seize_selected() %>%
    set_prioritization(c(0,-1,-1)) %>% ##cancel queueing
    renege_abort() %>% 
...
saveRDS(resources,"../Model Outputs/7e-resources.rds")
saveRDS(attribs,"../Model Outputs/7e-attribs.rds")
saveRDS(arrivals,"../Model Outputs/7e-arrivals.rds")
...
resources2<-filter(resources,resource=="Periop")
...
print(ggplot(resources2,aes(x=rdate,y=server,color=replication)) + geom_point(alpha=0.1,shape=16) + scale_color_gradient(low="blue", high="red") + stat_summary(aes(x=rwdate,y=server),fun.data="mean_sdl",geom="smooth",se=TRUE)  + xlim(start_date,end_date)  +labs(x="Date",y="Beds") + theme_bw(base_size=16) )


#print(plot(resources,metric="usage",c("ICU","21CC"),steps=TRUE,items=c("server","queue")))
#print(plot(resources,metric="usage",c("Periop"),steps=TRUE,items=c("server","queue")))
#print(plot(resources,metric="usage",c("Periop"),items=c("server","queue")))
